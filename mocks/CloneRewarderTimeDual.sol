...
contract CloneRewarderTimeDual is IRewarder,  BoringOwnable{
    ...
    // All information of token 1
    IERC20 public rewardToken1;
    // All information of token 2
    ...
    struct UserInfo {
        uint256 amount; //balance of UserInfor
        uint256 rewardDebt1; // number reward token of token 1
        uint256 rewardDebt2; // number reward token of token 2
        uint256 unpaidRewards1; // unpaid Reward 1
        uint256 unpaidRewards2; // unpaid Rewards 2
    }
    
    /// @notice Info of the rewarder pool.
    struct PoolInfo {
        uint128 accToken1PerShare; //token1 per shear (per time update)
        uint128 accToken2PerShare; // token2 per share (per time update)
        uint64 lastRewardTime; // lat reward time
    }
    ...
    // Save reward per secound 1
    uint128 public rewardPerSecond1;
    // Save reward per secound 2
    uint128 public rewardPerSecond2;
    // Save master liquidity pool
    IERC20 public masterLpToken;
    // Save variable ACC_TOKEN_PRECISION = 1e12
    uint256 private constant ACC_TOKEN_PRECISION = 1e12;
    // Save addredd of smart contract Masterchef_v2
    address public immutable MASTERCHEF_V2;
    // Save variable unclock = 1 => Locked else unclock = 2 => Unlock
    ...
    //Set address for token MasterchefV2
    ...
    function onSushiReward (uint256 pid, address _user, address to, uint256, uint256 lpTokenAmount) onlyMCV2 lock override external {
        ...
        // Get information poolInfo
        PoolInfo memory pool = updatePool(pid);
        // get informationm UserInfo
        UserInfo memory _userInfo = userInfo[pid][_user];
        // Save pendind token1
        uint256 pending1;
        // Save peding token2
        uint256 pending2;
        // Set information UserInfor amd PoolInfo
    ...
    function reclaimTokens(address token, uint256 amount, address payable to) public onlyOwner {
        ...
        if (token == address(0)) {
            //Transfer amount to address (0)
            to.transfer(amount);
        } else {
            //Encode value insave trasnfer    
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        pool = poolInfo[pid];
        // Check Blocktime greater than PoolInfor.lastRewaedTime
        if (block.timestamp > pool.lastRewardTime) {
            // Get balance ofsmart contract MasterchefV2
            uint256 lpSupply = IMasterChefV2(MASTERCHEF_V2).lpToken(pid).balanceOf(MASTERCHEF_V2);
            // Check balance of totak supply
            if (lpSupply > 0) {
                // Get lengthof time to activate
                uint256 time = block.timestamp.sub(pool.lastRewardTime);
                // Pedding of token 1
                uint256 pending1 = time.mul(rewardPerSecond1);
                // Pedding of token 2
