...
import "./libraries/SignedSadefeMath.sol";
...
contract MiniChefV2 is BoringOwnable, BoringBatchable {
    ...
    function pendingSushi(uint256 _pid, address _user) external view returns (uint256 pending) {
        ...
        // Get The informayion of User
        UserInfo storage user = userInfo[_pid][_user];
        // Get number token sushi share per time (seconds)
        uint256 accSushiPerShare = pool.accSushiPerShare;
        // Get total supply (balance of lpToken Address)
    ...
    function deposit(uint256 pid, uint256 amount, address to) public {
        //Get information of poolInfo
        PoolInfo memory pool = updatePool(pid);
        //Get information of userInfo
        UserInfo storage user = userInfo[pid][to];

        // Effects add amount for user.amount
        user.amount = user.amount.add(amount);
        // Effect add amount rewardDebt for user
        ...
        // Tranfer amount of user sender  to liquidity provided
    ...
    function withdraw(uint256 pid, uint256 amount, address to) public {
        ...
        // Effects subtract from rewardDebt with value (calculate value in pool)
        user.rewardDebt = user.rewardDebt.sub(int256(amount.mul(pool.accSushiPerShare) / ACC_SUSHI_PRECISION));
        // Effect subtract from amount in user amount 
        ...
        //Transfer token app token to admin
